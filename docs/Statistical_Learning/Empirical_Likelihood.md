...
经验似然方法是 Owen 提出的一种非参数统计推断方法,不需要假定数据来自一个具体的分布族.
...
非参数似然的最大值点就是ECDF, 因此我们可以称ECDF是非参数的极大似然估计. 考虑一个原假设为 $$H_0:T(F_0)=\theta_0$$的检验,我们定义其 profile的似然比函数为：
...
> 假设 $$X_1,\ldots,X_n$$ 是独立同分布的随机变量，其分布函数为 $$F_0$$. 令 $$\mu_0=E(X_i)$$, 并且假设$$0<Var(X_i)<\infty$$. 那么当$$n\to \infty$$时, $$-2\log(R(\mu_0))$$会依分布收敛到一个卡方分布，其自由度为1. 

这可以来帮我们选择假设检验问题（和置信区间）中的临界值，当显著性水平为 $$\alpha$$ 时, 如果 $$-2\log R(\mu_0)>\chi^{2,1-\alpha}_{(1)}$$，我们就拒绝原假设. 从证明的细节，和一些模拟的结果来看，如果将临界值$$\chi_{(1)}^{2,1-\alpha}$$ 替换为$$F_{1,n-1}^{1-\alpha}$$，效果会更好.有了临界值，我们就可以构造均值的置信区间了,其具体形式如下:
...
$$X$$的分布函数是 $$F$$, $$F(A)$$ 的意思是 $$Pr(X\in A), A\subset \mathbb{R}^d$$. 

让 $$\delta_x$$ 表示以概率1，$$X=x$$的一个分布，$$\delta_x(A)=1_{x\in A}$$. 

则 $$X_1,\ldots,X_n$$的经验累积分布函数为
$$
...
> 假设 $$X_i,\ldots,X_n\in \mathbb{R}^d$$ 独立同分布于一个共同的分布函数 $$F_0$$. 对于 $$\theta_0 \in \Theta \subseteq \mathbb{R}^p$$, $$X\in \mathbb{R}^d$$ , $$m(X,\theta)\in \mathbb{R}^s$$. 令 $$\theta_0 \in \Theta$$ 并且使得 $$Var(m(X_i,\theta_0))$$ 有限,并且秩是 $$q>0$$. 当 $$\theta_0$$ 满足 $$E(m(X,\theta_0))=0$$时,
...
我们不考虑结的情况，并且将分位数的定义简化为$$E(1_{X\le Q^{\alpha}}-\alpha)=0.$$

结点的存在也基本不影响最后的结论，具体关于结点的分析参见 Owen(2001) 对应的章节分析. 

我们定义:$$X_0=-\infty,X_{n+1}=\infty$$ 已确保可以包括$$Q^{\alpha}<X_{(1)}$$和$$Q^{\alpha}\ge X_{(n)}$$的情况.
...
-   当$$X_{(1)}<q\le X_{(n)}$$, 经验似然比方程为
    ...
    $$
    R(p,q)=(\dfrac{p}{\hat{p}})^{n\hat{p}}(\dfrac{1-p}{1-\hat{p}})^{n(1-\hat{p})}.
    $$
...
假设 $$(X,Y)\in \mathbb{R}^2$$,并且我们知道$$X$$的均值,我们想要构造$$Y$$的均值的估计和置信区间. 我们定义条件经验似然比函数如下.
...
> 假设 $$(X_i,Y_i)\in \mathbb{R}^{p+q}$$是独立同分布的随机向量,其共同分布为$$F_0$$, 均值是 $$(\mu_{x0},\mu_{y0})$$,方差矩阵满秩，秩为 $$p+q$$. 那么随着 $$n \to \infty$$, $$-2\log R_{Y|X}(\mu_{y0}|\mu_{x0})$$依分布收敛到$$\chi^2_{(q)}$$ .
